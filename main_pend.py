...
def train3dof(configs):
    ...
    #half = int(dim/6) 
    model = rollout_GNN_GRU(graph,2,128,8,["tanh","relu"," "],bias=BIAS,type = MODEL,dropout=0.85)
    #model = GNN_maker_HNN(graph,2,128,6,["tanh",""],type=MODEL,bias=BIAS)
    ...
    torch.save(model.state_dict(),dic+"server_3dof.pth")
    torch.save(opti.state_dict(),dic+"server_opti.pth")
    torch.save(container,dic+"losses.pt")
    model.eval()
    torch.save(model,dic+"model.pt")
    torch.save(eval,dic+"eval.pt")
    torch.save(H,dic+"eval_H.pt")
...
def train4dof(configs):
    ...
    if os.path.isfile(dic+"server_3dof.pth"):
        print("loading prevoius model")
        model = load_model(model,dic+"server_3dof.pth")
    ...
    if OPTI == "adamW":
        opti = torch.optim.AdamW(model.parameters(),lr=LR)
    if os.path.isfile(dic+"server_3dof.pth"):
        opti.load_state_dict(torch.load(dic+"server_opti.pth"))
    ...
    dic4 = "res_4dof/"
    torch.save(model,dic4+"model.pt")
    torch.save(container,dic4+"losses.pt")
    torch.save(eval,dic4+"eval.pt")
    torch.save(H,dic4+"eval_H.pt")
